...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, bad-amulet curse gate, and the ordinary shopkeeper/stocking/mimic front doors now match through the current evidence. The next seed5002 mismatch is `FR 4478`, expected `rn2(1000)` while JS emits `rn2(100)`, indicating a later shop-stock object-init or stocking-position detail.
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 4/410 R 5817/12167`; first local mismatch is still the post-startup one-cell gold color attribute after Enter, with local RNG mismatch `FR 4478` (`rn2(1000)` expected vs `rn2(100)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
