...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wish lookup, wizard `^G` create-monster prompt/placement, and the ordinary shopkeeper/stocking/mimic front doors now match through the current evidence. The next seed5002 mismatch is wand-of-fire bounce/hit message output, while RNG first diverges at the later `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual).
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 88/410 R 5934/12167`; first local mismatch is wand-of-fire bounce/hit message output (`The bolt of fire bounces!  The bolt of fire hits you!--More--` expected, blank actual), with local RNG mismatch `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wishes, wizard `^G` named monster creation, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
