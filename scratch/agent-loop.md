...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 128/410 R 6024/12167`; first local mismatch is segment-2 map/layout state after the first command, with local RNG mismatch still `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual). Startup inventory, no-legacy AC, no-legacy welcome width packing, positive `vary_init_attr()` max handling, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wishes, wizard `^G` named monster creation, current fire-wand ray display/message front door, deferred cloak scorch/invisibility/oil destruction and death prompt handling, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
