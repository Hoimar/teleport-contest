...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wish lookup, wizard `^G` create-monster prompt/placement, fire-wand bounce/hit front door, and the ordinary shopkeeper/stocking/mimic front doors now match through the current evidence. The next seed5002 mismatch is fire side-effect messaging for worn cloak and inventory potion destruction, while RNG first diverges at the later `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual).
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 103/410 R 5908/12167`; first local mismatch is fire side-effect messaging (`Your cloak smoulders!  Your potion of invisibility boils and explodes!--More--` expected, blank actual), with local RNG mismatch still `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wishes, wizard `^G` named monster creation, current fire-wand ray display/message front door, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
