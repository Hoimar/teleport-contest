...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, and the helm-of-brilliance `SPFX_NOGEN` artifact miss now match through the evidence. The next seed5002 mismatch is `FR 4117`, expected `rnd(2)` while JS emits `rn2(1000)`, indicating a later `mkobj()`/room-fill branch after the armor erosion path rather than a startup UI issue.
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 4/410 R 4144/12167`; first local mismatch is a post-startup map/cursor divergence after Enter, with local RNG mismatch `FR 4117` (`rnd(2)` expected vs `rn2(1000)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, and the `SPFX_NOGEN` helm artifact miss now match through the current evidence.
