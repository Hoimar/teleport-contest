...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, and the ordinary shopkeeper/stocking/mimic front doors now match through the current evidence. The next seed5002 mismatch is visible inventory message text for a wand (`o - a pine wand.` expected, blank actual), while RNG still first diverges at the later `FR 5817` shop-stock/object-init boundary.
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 72/410 R 5836/12167`; first local mismatch is an inventory message blank where C displays `o - a pine wand.`, with local RNG mismatch still `FR 5817` (`rn2(41)` expected vs `rn2(12)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
