...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wish lookup, wizard `^G` create-monster prompt/placement, fire-wand bounce/hit front door, deferred cloak scorch and invisibility-potion destruction, and the ordinary shopkeeper/stocking/mimic front doors now match through the current evidence. The next seed5002 mismatch is the next fire inventory-destruction side-effect message (`Your potion of oil ignites and explodes!`), while RNG first diverges at the later `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual).
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 112/410 R 5915/12167`; first local mismatch is fire side-effect messaging (`Your potion of oil ignites and explodes!--More--` expected, blank actual), with local RNG mismatch still `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wishes, wizard `^G` named monster creation, current fire-wand ray display/message front door, deferred cloak scorch/invisibility-potion destruction, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
