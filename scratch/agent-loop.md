...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 4/410 R 3986/12167`; first local mismatch is a post-startup map/cursor divergence after Enter, with local RNG mismatch `FR 3961` (`rn2(12)` expected vs `rn2(2)` actual). Startup inventory, no-legacy AC, date messages, and `moveloop_preamble()` now match through the current evidence.
