...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wish lookup, wizard `^G` create-monster prompt/placement, fire-wand bounce/hit front door, deferred cloak scorch/invisibility/oil destruction and death prompt handling, and the ordinary shopkeeper/stocking/mimic front doors now match through the current evidence. The next seed5002 mismatch is after the current death prompt boundary, while RNG first diverges at the later `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual).
...
- Target verification after bad-amulet curse gate: `seed5002` advanced from `R 5787/12167` to `R 5817/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`.
- Target verification after erodeproof grease gate: `seed5002` advanced from `R 5817/12167` to `R 5836/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`.
- Full-suite verification after placed-gold metadata color: public screens moved from `S 331/11406` to `S 399/11406`, `1/44 passing`; `seed5002` advanced from `S 4/410` to `S 72/410` at the same `R 5836/12167`. Sentinel remains stable at `S 326/1063 R 35782/64569`.
- Target verification after named-wand wish lookup: `seed5002` advanced from `S 72/410 R 5836/12167` to `S 75/410 R 5889/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`.
- Target verification after wizard `^G` monster creation: `seed5002` advanced from `S 75/410 R 5889/12167` to `S 88/410 R 5934/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`.
- Target verification after fire-wand ray front door: `seed5002` advanced from `S 88/410 R 5934/12167` to `S 103/410 R 5908/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`. The lower lagging RNG prefix is classified as expected from adding the live `ubuzz()` consumer while the known earlier `FR 5827` shop/object mismatch remains unresolved.
- Target verification after deferred fire side effects: `seed5002` advanced from `S 103/410 R 5908/12167` to `S 112/410 R 5915/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`. The next visible mismatch is the oil-potion explosion message, with the same earlier `FR 5827` shop/object mismatch.
- Full-suite verification after deferred fire side effects: public screens moved from `S 399/11406` to `S 467/11406`, `1/44 passing`; `seed5002` strict screens are `S 111/410`, cells `S 112/410`, `R 5915/12167`.
- Target verification after oil explosion/death prompt chain: `seed5002` advanced from `S 112/410 R 5915/12167` to `S 124/410 R 6024/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`. The first local screen mismatch is now after the wizard-mode death prompt boundary, with the same earlier `FR 5827` shop/object mismatch.
- Target verification after no-legacy welcome width packing: `seed5002` remains at `S 124/410 R 6024/12167`, but segment-2 startup now clears the stale `--More--`/cursor drift and exposes the real male-Wizard status mismatch (`Wi:10` expected vs `Wi:8` actual). Sentinel remains stable at `S 326/1063 R 35782/64569` after preserving legacy quest-intro welcome queuing.
- Target verification after positive `vary_init_attr()` max handling: `seed5002` advanced from `S 124/410 R 6024/12167` to `S 128/410 R 6024/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`. Segment-2 male Wizard Wisdom now matches, exposing a real map/layout mismatch after the first command.
- Full-suite verification after welcome/attribute startup fixes: strict public screens are `S 453/11406`, `1/44 passing`; `seed5002` strict screens are `S 126/410`, cells `S 128/410`, `R 6024/12167`. Sentinel remains stable at `S 326/1063 R 35782/64569`.
- Target verification after charged starting rings and dynamic themed `Storeroom` fill front door: `seed5002` remains at `S 128/410` but RNG advances from `R 6024/12167` to `R 6044/12167`; sentinel remains stable at `S 326/1063 R 35782/64569`. Segment-2 startup RNG now reaches the storeroom mimic appearance/statue-object sequence instead of the starting-ring adjustment, and the special-level mimic path no longer runs ordinary minvent gates.
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 128/410 R 6044/12167`; first local mismatch is segment-2 map/layout state after the first command, with local RNG mismatch still `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual). Startup inventory including charged-ring `spe`, no-legacy AC, no-legacy welcome width packing, positive `vary_init_attr()` max handling, dynamic themed `Storeroom` fill front door and no-minvent mimic creation, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wishes, wizard `^G` named monster creation, current fire-wand ray display/message front door, deferred cloak scorch/invisibility/oil destruction and death prompt handling, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
