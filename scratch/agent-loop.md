...
- Active subsystem hypothesis: startup inventory, date-message preamble, and the current human wererat statue container gate now match through the evidence. The next seed5002 mismatch is `FR 4091`, expected `rn2(100)` while JS emits `rn2(5)`, indicating a later `mkobj()`/room-fill branch after the statue container path rather than a startup UI issue.
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 4/410 R 4114/12167`; first local mismatch is a post-startup map/cursor divergence after Enter, with local RNG mismatch `FR 4091` (`rn2(100)` expected vs `rn2(5)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, and the human-form wererat statue `verysmall()` gate now match through the current evidence.
