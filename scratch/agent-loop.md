...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, and the first ordinary shopkeeper/stocking pass now match through the evidence. The next seed5002 mismatch is `FR 4149`, expected `rn2(10)` while JS emits `rn2(17)`, indicating remaining shop-stock object-init detail after the first mimic path rather than missing shop dispatch.
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 4/410 R 4256/12167`; first local mismatch is a post-startup one-cell gold color attribute after Enter, with local RNG mismatch `FR 4149` (`rn2(10)` expected vs `rn2(17)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, and first ordinary shopkeeper/stocking path now match through the current evidence.
