...
- Active subsystem hypothesis: startup inventory, date-message preamble, the current human wererat statue container gate, the helm-of-brilliance `SPFX_NOGEN` artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wish lookup, and the ordinary shopkeeper/stocking/mimic front doors now match through the current evidence. The next seed5002 mismatch is the wizard-mode `^G` create-monster prompt, while RNG first diverges at the later `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual).
...
2. Continue mklev/object generation from current evidence:
   - `seed5002` now reaches `S 75/410 R 5889/12167`; first local mismatch is the wizard-mode `^G` create-monster prompt (`Create what kind of monster?` expected, `Unknown command '^G'.` actual), with local RNG mismatch `FR 5827` (`rn2(51)` expected vs `rn2(41)` actual). Startup inventory, no-legacy AC, date messages, `moveloop_preamble()`, the human-form wererat statue `verysmall()` gate, `SPFX_NOGEN` helm artifact miss, bad-amulet curse gate, erodeproof grease gate, placed-gold display color, named-wand wishes, first ordinary shopkeeper/stocking path, and shop mimic appearance branch now match through the current evidence.
