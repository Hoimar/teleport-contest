...
function usage() {
    return 'Usage: node --loader ./scratch/pet-kill-trace-loader.mjs scratch/trace-pet-kill.mjs <session.json>';
}

const [sessionPath] = process.argv.slice(2);
if (!sessionPath) throw new Error(usage());

globalThis.__teleportPetKillTrace = true;
globalThis.__teleportPetKillEvents = [];
...
for (const ev of globalThis.__teleportPetKillEvents || []) {
