...
| Debug tooling convention | — | One-off scripts in `scratch/` or `debug/`; batch triage in `scripts/triage-corpus.mjs`; dog-goal trace in `scripts/trace-dog-goal.mjs` | 🟡 | Temp debug scripts MUST be deleted after use. Removed dead tracked `scratch/run_test.mjs` stub; remaining tracked scratch scripts are diagnostics. `scripts/triage-corpus.mjs` regenerates `scratch/divergence-inventory.md` from compact triage data for subsystem-level planning. `scripts/trace-dog-goal.mjs` runs a session prefix and prints the tame pet, C-shaped dog-goal search rectangle, floor objects, inventory, and RNG window for pet/object parity work. Never leave `console.log` or `import('fs').writeFileSync` in production paths (jsmain.js, allmain.js). |
...
| Per-step monster movement / regen RNG | `monmove.c`, `regen.c` | `js/monmove.js` + partial turn consumers | 🟡 | Per-step replay is no longer called from `moveloop_core()`, and the dead `fastforward_step()` exports have been removed from the seed replay modules. `makemon()` now preserves C `fmon` head-insertion order, `mcalcmove()` handles slow/fast speed states, and fog clouds consume their `m_everyturn_effect()` vapor-cloud TTL from the `movemon_singlemon()` phase before the movement-budget check. Movement AI remains skeletal; `seed8000` now blocks at FR 2985 where C still has another monster ready for `distfleeck()` while JS begins the next movement allocation. `seed0383` now crosses the fog-cloud turn effect and movement allocation, then blocks in pet `dog_goal()` object visibility/apport ordering at FR 9713. |
...
| Object placement on map | `mkobj.c:mksobj()` | `js/mklev.js:mksobj_at()` / `mkobj_at()` + legacy hardcoded arrays in `allmain.js` | 🟡 | Generated `_at` object creation now retains non-gold objects on `level.objects` with class glyph state, uses C's newest-first `fobj` order, and `mkcorpstat()` now places created corpses/statues at the supplied coordinates instead of only consuming their init RNG. `mktrap_victim()` now retains the trap item, cursed random possessions, optional candle, and corpse at the trap square; PIT/landmine `breaktest()` destruction remains pending. Seed0002 still has legacy hardcoded startup objects, and object stacking/descriptions/material swaps/container contents remain incomplete. Evidence: `seed0116` dog blocker moved to `dog_goal()` object reachability/resistance rather than missing the pet wanderer gate; retained corpse/statue/trap-victim state and C-like floor-list order reduce hidden object-state debt for later dog/display/pickup work. |
...
| Combat | `fight.c` | — | 🔴 | |
| Pet behavior | `dog.c`, `dogmove.c` | `js/dog.js` + `js/mklev.js:makemon()` + `js/monmove.js` (partial) | 🟡 | Starting pet creation is partially ported for configured non-replay roles: role/preferred-pet selection, `MM_EDOG|NO_MINVENT`, near-hero `collect_coords()` ring shuffling, pet tame/peaceful setup, pre-inventory `edog->apport` initialization, and kitten/little-dog/pony metadata. Tame monster turns now enter a partial `dog_move()` path with the upstream kitten/pony `M2_WANDER` gate, C-like neighbor scan order, and C's `chcnt` reset after strictly better candidates. `dog_goal()` now scans retained floor objects and hero inventory with `dogfood()`/`obj_resists()` guards for real numeric `otyp` objects; its apport branch has the C lighting/front-door predicate, `m_cansee()` via `clear_path()`, partial diet-aware food classification, rock-class `UNDEF` handling after the resistance roll, and pool/lava/boulder reachability gate. Level-teleport pet arrival now preserves the migrating tame monster's movement/data across `mklev()` instead of recreating a fixed-movement pet. Current `seed0116` blocker is missing three object-resistance calls after retained Wizard inventory, likely nearby floor/shop object state; current `seed0383` blocker is missing additional retained/positioned floor objects after the second apport check at FR 9716. Hunger, naming, saddles, full object fetching/eating, migration details, and interactions remain incomplete. |
...
1. **Complete `dog_goal()` object visibility/state (`dogmove.c`)** — `seed0383` now reaches FR 9716 after fog-cloud, movement-allocation, object-order, pre-inventory apport, clear-path visibility, and partial pet food/reachability parity; the next mismatch is missing floor objects/positions in the remaining `fobj` scan. `seed0116` still needs three missing `obj_resists()` calls before the pet chooses its square, likely incomplete floor/shop object state. Prepare this with a reusable diagnostic that prints the C-shaped floor/inventory object scan rather than adding another movement probe.
2. **Classify remaining special-level monster/list effects** — `seed0383` crossed the prior fog-cloud phase-order blocker, but special-level monster ordering can still affect later movement and display once `dog_goal()` advances.
