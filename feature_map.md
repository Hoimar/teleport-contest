...
1. **Classify `seed0383` FR 10154 iguana spatial drift** — `seed0383` now reaches FR 10154 after command, delayed wear/occupation, inventory-stack, dog-goal, pet ranged scoring, fog-region aging, peaceful/light front doors, flying `mfndpos()` terrain accounting, the first pet melee/gnome kill, and adjusted-level pet attack balking. The next mismatch is C neutral `dog_move()` candidate selection versus JS pet melee against an iguana at `(27,4)`. Focus on the iguana's prior ordinary movement/list state, not another pet attack suppression.
