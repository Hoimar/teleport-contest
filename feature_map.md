...
| Per-step monster movement / regen RNG | `monmove.c`, `regen.c` | `js/monmove.js` + partial turn consumers | 🟡 | Per-step replay is no longer called from `moveloop_core()`, and the dead `fastforward_step()` exports have been removed from the seed replay modules. `makemon()` now preserves C `fmon` head-insertion order, `mcalcmove()` handles slow/fast speed states, and fog clouds consume their `m_everyturn_effect()` vapor-cloud TTL. Movement AI remains skeletal; `seed8000` now blocks at FR 2985 where C still has another monster ready for `distfleeck()` while JS begins the next movement allocation. `seed0383` now shows fog-cloud TTL in the wrong mcalcmove slot, pointing at special-level monster list/order drift. |
...
| Combat | `fight.c` | — | 🔴 | |
| Pet behavior | `dog.c`, `dogmove.c` | `js/dog.js` + `js/mklev.js:makemon()` + `js/monmove.js` (partial) | 🟡 | Starting pet creation is partially ported for configured non-replay roles: role/preferred-pet selection, `MM_EDOG|NO_MINVENT`, near-hero `collect_coords()` ring shuffling, pet tame/peaceful setup, and kitten/little-dog/pony metadata. Tame monster turns now enter a partial `dog_move()` path with the upstream kitten/pony `M2_WANDER` gate and C-like neighbor scan order. `dog_goal()` now scans retained floor objects and hero inventory with `dogfood()`/`obj_resists()` guards for real numeric `otyp` objects. Level-teleport pet arrival now preserves the migrating tame monster's movement/data across `mklev()` instead of recreating a fixed-movement pet. Current `seed0116` blocker is missing three object-resistance calls after retained Wizard inventory, likely nearby floor/shop object state. Hunger, naming, saddles, full object fetching/eating, migration details, and interactions remain incomplete. |
...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `5788/16915` to `9884/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init, pet-arrival movement, and fog-cloud every-turn vapor TTL are now modeled. Current blocker is still FR 9666 (`rn2(3)` expected vs `rn2(12)` actual), now classified as monster list/order drift: JS emits the fog-cloud `create_gas_cloud()` TTL later in the `mcalcmove()` sequence than C. |
...
1. **Classify special-level monster list order** — `seed0383` now reaches FR 9666 after bigroom special loading, arrival, and wizard wish object initialization. JS has the fog-cloud `create_gas_cloud()` TTL, but it appears later in the `mcalcmove()` sequence, so compare C/JS `fmon` order around pet arrival and bigroom group insertion.
