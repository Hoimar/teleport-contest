...
| Combat | `fight.c`, `mhitm.c` | `js/dog.js` (pet-only front door) | 🔴 | Hero combat and full monster-vs-monster combat remain unimplemented. A narrow pet `mattackm()` front door now consumes the current physical miss/passive and hit/damage/death/growth RNG shape for `dogmove.c` evidence, but this is not a general combat port. |
...
1. **Classify `seed0383` FR 9933 pet-vs-monster spatial state** — `seed0383` now reaches FR 9933 after command, inventory-stack, dog-goal, pet ranged scoring, fog-region, peaceful/light front doors, flying `mfndpos()` terrain accounting, and the first pet melee front door. The next mismatch is C `mattackm()` vs JS `score_targ()` during the pet's second movement pass, pointing at live monster/pet position state before ranged scoring.
