...
| Per-step monster movement / regen RNG | `monmove.c`, `regen.c`, `were.c` | `js/monmove.js` + partial turn consumers | 🟡 | Per-step replay is no longer called from `moveloop_core()`, and the dead `fastforward_step()` exports have been removed from the seed replay modules. `makemon()` now preserves C `fmon` head-insertion order, `mcalcmove()` handles slow/fast speed states, `mcalcdistress()` owns the human-form werecreature `were_change()` denominator gate before movement allocation, fog clouds retain/age vapor regions so `m_everyturn_effect()` only consumes TTL RNG when no gas region is already present, uppercase run commands reuse the normal turn advancement loop, sleeping/frozen monsters and object/furniture-appearing hiders spend movement without entering `distfleeck()`, and ordinary non-pet monsters now take a minimal `dochug()`/`m_move()` path with post-`m_move()` `distfleeck()`, peaceful/light approach gates, `mtrack` backtracking rolls, and partial `mfndpos()` terrain eligibility for flying/swimming/pool/lava squares. Movement AI remains skeletal, but `seed0116` now has full RNG parity (`12562/12562`) and only a display attr mismatch remains. `seed8000` remains complete on screens, and `seed0383` now blocks at FR 9933 where the second pet movement pass enters ranged scoring while C enters `mattackm()`. |
...
| seed0002-healer-reflection-drummer | 11 | 595 | 2026-05-12 | First visible screen mismatch unchanged at screen 11. Current lagging RNG evidence is `1249/27158` after generalized movement/object-state fixes. |
...
| seed0116-wizard-wear-shop | 109 | 127 | 2026-05-12 | Door-state aliasing, tutorial prompt lifecycle, rest command, debug `^V` prompt/Enter handling, static themed/Soko `des.map()`, irregular region bounds, corridor/vault accounting, random shop decision, level-teleport arrival placement, pet migration, normal Space handling, basic wear/takeoff/pickup/drop prompts, retained mineral objects, Soko zoo room/fill, leprechaun gold/minvent state, `rnd_misc_item()`, spider/snake hide-under objects, Soko mimic appearance state, sleeping/hider movement front doors, pet wanderer short-circuiting, partial `dog_goal()` object/inventory scans, Sokoban boulder avoidance, `whappr`, uppercase `L` run handling, ordinary non-pet `m_move()`/`mtrack` ownership, persistent inventory letters, wished ring/digging-wand retention, `P` ring put-on prompts, `z` wand selection/direction flow, room sound gates, and lycanthrope distress moved this evidence path to screen 109 with complete RNG parity (`12562/12562`). Current visible mismatch is an attr-only Soko map/object color drift at screen 109. |
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-12 | Current lagging RNG is `10177/16915`; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init and retained wish inventory, `#wizintrinsic` hallucination prompt state, initial inventory stack merging, pet-arrival movement, fog-cloud every-turn vapor region retention, newest-first floor-object order, pre-inventory pet apport, `m_cansee()` clear-path visibility, partial pet food/reachability gates, pet ranged target scoring, ordinary monster movement front doors, flying/swimming `mfndpos()` terrain candidates, pet candidate melee front door, and hero occupied-monster movement front door are now modeled. Current blocker is still FR 9933: JS has no monster on the westward `h` target before the C hero attack screen, then the second pet movement pass reaches `score_targ()` `rnd(5)` with no adjacent target while C enters `mattackm()`; the next owner is live monster spatial state before the hero attack/pet second pass, not `dog_goal()` object scans. |
...
1. **Classify `seed0383` FR 9933 pre-attack monster spatial state** — `seed0383` now reaches FR 9933 after command, inventory-stack, dog-goal, pet ranged scoring, fog-region, peaceful/light front doors, flying `mfndpos()` terrain accounting, and the first pet melee front door. The next mismatch is C `mattackm()` vs JS `score_targ()` during the pet's second movement pass; screen/message evidence also shows C has a monster on the westward `h` target where JS has empty floor. Focus on ordinary monster position/movement budgets before the hero attack and second pet pass.
2. **Classify Soko screen 109 attr-only map drift** — seed0116 now has full RNG parity but still mismatches on object/monster colors at screen 109. Treat this as display/object-state debt rather than movement.
