...
1. **Classify `seed0383` FR 10360 tiger `mtrack`/spatial drift** — `seed0383` now matches the ice-vortex `AT_ENGL`/`gulpmu()` sequence, the damaged-hero regeneration roll, swallowed pet `dog_goal()` object resistance scan, candidate selection, and pet ranged scoring. The next mismatch is C `distfleeck()` versus JS tiger `m_move()` `mtrack` backtracking (`rn2(32)`), so compare prior tiger movement, `mtrack[0]`, candidate geometry, and spatial/list state before changing the selector.
