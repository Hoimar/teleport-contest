...
| seed0002-healer-reflection-drummer | 11 | 595 | 2026-05-12 | First visible screen mismatch unchanged at screen 11. Current lagging RNG evidence is `1259/27158` after generalized movement/object-state fixes. |
...
1. **Complete `m_move()` candidate geometry (`monmove.c:mfndpos`)** — `seed0383` now reaches FR 9806 after command, inventory-stack, dog-goal, pet ranged scoring, fog-region, peaceful, and light/bat/stalker movement front-door parity. The next mismatch is `rn2(28)` expected vs `rn2(16)` actual in backtracking/candidate selection, pointing at missing `mfndpos()` terrain/flag candidates rather than dog-goal object scans.
