...
1. **Port `seed0383` FR 10213 monster-to-hero combat front door** — `seed0383` now reaches FR 10213 after command, delayed wear/occupation, inventory-stack, dog-goal, pet ranged scoring, fog-region aging, peaceful/light front doors, flying `mfndpos()` terrain accounting, pet kill `grow_up()` HP state, and adjusted-level pet attack balking. The next mismatch is C `mattacku()` (`rnd(2)`, `rnd(20)`, hit damage, knockback, and status updates) versus JS ordinary movement `distfleeck()`. Focus on `monmove.c:dochug()` adjacent hero attack and `mhitu.c`, not another pet selector change.
