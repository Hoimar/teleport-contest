...
1. **Classify `seed0383` FR 10280 ice-vortex `AT_ENGL` timing** — `seed0383` now reaches FR 10280 after the first adjacent ape `mattacku()` physical multiattack, delayed armor final-turn AC, wish BUC/enchantment state, dog-goal, pet ranged scoring, fog-region aging, and pet grow-up/balk behavior. The next mismatch is C ice-vortex `AT_ENGL`/`gulpmu()` (`rnd(2)`, hit, `d(1,6)`, swallow timer, cold gate) versus JS movement allocation. A direct engulf front door fired early at FR 10183 because JS already had the vortex adjacent; classify vortex spatial/list timing before enabling `gulpmu()` broadly.
