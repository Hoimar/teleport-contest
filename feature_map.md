...
| Combat | `fight.c` | — | 🔴 | |
| Pet behavior | `dog.c`, `dogmove.c` | `js/dog.js` + `js/mklev.js:makemon()` + `js/monmove.js` (partial) | 🟡 | Starting pet creation is partially ported for configured non-replay roles: role/preferred-pet selection, `MM_EDOG|NO_MINVENT`, near-hero `collect_coords()` ring shuffling, pet tame/peaceful setup, and kitten/little-dog/pony metadata. Tame monster turns now enter a partial `dog_move()` path with the upstream kitten/pony `M2_WANDER` gate and C-like neighbor scan order. `dog_goal()` now scans retained floor objects and hero inventory with `dogfood()`/`obj_resists()` guards for real numeric `otyp` objects; its apport branch has the C lighting/visibility front door, partial diet-aware food classification, and pool/lava/boulder reachability gate. Level-teleport pet arrival now preserves the migrating tame monster's movement/data across `mklev()` instead of recreating a fixed-movement pet. Current `seed0116` blocker is missing three object-resistance calls after retained Wizard inventory, likely nearby floor/shop object state; current `seed0383` blocker moved to FR 9715 after non-swimming pets stopped treating pool food as reachable apport candidates. Hunger, naming, saddles, full object fetching/eating, migration details, and interactions remain incomplete. |
...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `5788/16915` to `9916/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init, pet-arrival movement, fog-cloud every-turn vapor TTL, newest-first floor-object order, and partial pet food/reachability gates are now modeled. Current blocker moved from FR 9666 to FR 9715: after matching fog-cloud TTL, all 36 movement-allocation `rn2(12)` rolls, random-monster/gethungry/slippery-finger gates, and the first `dog_goal()` object scan boundary, JS now reaches the apport roll but diverges on the following movement/allocation call. |
...
1. **Complete `dog_goal()` object visibility/state (`dogmove.c`)** — `seed0383` now reaches FR 9715 after fog-cloud, movement-allocation, object-order, and partial pet food/reachability parity; the next mismatch is immediately after the first apport roll. `seed0116` still needs three missing `obj_resists()` calls before the pet chooses its square, likely incomplete floor/object state.
