...
| Command dispatch (`rhack`) | `cmd.c:rhack()` | `js/cmd.js:rhack()` | 🟡 | Move commands, uppercase vi run commands, rest/wait (`.`), search (`s`), empty-square pickup messaging, `#pay` no-shopkeeper message, basic drop, put-on/wear prompts for rings, amulets, and delayed armor evidence, zap-wand prompt/direction flow, wizard wishes for current object evidence, zero-time `#wizintrinsic` hallucination prompt state, debug `^V` numeric/menu level teleport, and `#levelchange` prompt/level-gain RNG are present. Run commands now consume repeated movement/turn loops before the next input boundary, but stop/turn/lookaround rules and full occupation continuation remain approximate. Most commands remain unimplemented. |
...
| Autopickup | `pickup.c` | — | 🔴 | |
| Inventory management | `invent.c` | `js/cmd.js` + `js/u_init.js` partial | 🟡 | Inventory is still not a full `invent.c` port, but carried objects now receive persistent `invlet` letters lazily, startup/wished stackable objects use conservative `addinv()`-style merge semantics, startup inventory clears cursed state before role adjustments, basic drop places the object on `fobj`, `P`/`W`/`z` prompt filters use object class and worn state rather than current array position, and worn startup cloak state feeds later `W`/`T` behavior. Evidence: `seed0116` now carries the wished teleport-control ring as `o` and the wished digging wand as `p`; `seed0383` Wizard duplicate scrolls now scan as one carried stack in `dog_goal()`, and delayed gray dragon scale mail selection advances the live movement evidence. |
...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-12 | Current lagging RNG is `10324/16915`; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init and retained wish inventory, zero-time `#wizintrinsic` hallucination prompt state, Wizard startup/worn-state handling, initial inventory stack merging, `P`/`W` wearable selection timing, pet-arrival movement, fog-cloud every-turn vapor region retention, newest-first floor-object order, pre-inventory pet apport, `m_cansee()` clear-path visibility, partial pet food/reachability gates, pet ranged target scoring, ordinary monster movement front doors, flying/swimming `mfndpos()` terrain candidates, pet candidate melee front door, and hero occupied-monster movement front door are now modeled. Current blocker is FR 10079 during the post-gnome-death pet turn: C continues `dog_move()` candidate choice with `rn2(1)` and then ranged scoring, while JS enters a pet melee `rnd(20)`, so the next owner is pet/monster spatial state or melee-candidate filtering after the first killed gnome. |
...
1. **Classify `seed0383` FR 10079 post-gnome-death pet spatial state** — `seed0383` now reaches FR 10079 after command, delayed wear, inventory-stack, dog-goal, pet ranged scoring, fog-region, peaceful/light front doors, flying `mfndpos()` terrain accounting, and the first pet melee/gnome kill. The next mismatch is C `dog_move()` candidate choice `rn2(1)` followed by ranged scoring versus JS entering pet melee `rnd(20)`. Focus on monster occupancy, pet candidate filtering, and post-kill spatial updates before suppressing or adding combat RNG.
