...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `5788/16915` to `9895/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init, pet-arrival movement, and fog-cloud every-turn vapor TTL are now modeled in the correct movement phase. Current blocker moved from FR 9666 to FR 9713: after matching fog-cloud TTL, all 36 movement-allocation `rn2(12)` rolls, random-monster/gethungry/slippery-finger gates, and five `dog_goal()` object-resistance checks, JS scans one more object where C rolls the apport `rn2(8)`. |
