...
|---|---|---|---|---|
| seed8000-tourist-starter | **23** | 23 | 2026-05-12 | Screen parity preserved. First RNG mismatch remains in live monster movement after generalized ordinary-monster movement work; current lagging RNG is `3076/3130`. |
| seed0002-healer-reflection-drummer | 11 | 595 | 2026-05-12 | First visible screen mismatch unchanged at screen 11. Current lagging RNG evidence is `1252/27158`; first RNG mismatch is FR 1215 after generalized movement/object-state fixes. |
| seed0116-wizard-wear-shop | 21 | 127 | 2026-05-12 | Door-state aliasing, tutorial prompt lifecycle, rest command, debug `^V` prompt/Enter handling, static themed `des.map()`, irregular region bounds, corridor/vault accounting, random shop decision, level-teleport arrival placement, pet migration, normal Space handling, basic wear/takeoff/pickup prompts, retained mineral objects, pet wanderer short-circuiting, partial `dog_goal()` object/inventory scans, `whappr`, uppercase `L` run handling, and ordinary non-pet `m_move()`/`mtrack` ownership moved this evidence path to screen 21 and FR 5710. Current blocker is pet apparent-nearby/position parity after the run: C consumes the kitten `M2_WANDER` gate while JS starts `dog_goal()` object resistance. Current lagging RNG is `5809/12562`. |
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-12 | Improved from `5788/16915` to `9937/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init and retained wish inventory, pet-arrival movement, fog-cloud every-turn vapor TTL, newest-first floor-object order, pre-inventory pet apport, `m_cansee()` clear-path visibility, partial pet food/reachability gates, and ordinary monster movement ownership are now modeled. Current blocker is FR 9716: after matching the kitten wanderer gate, six floor-object `obj_resists()` calls, and two failed apport rolls, JS reaches the follow-player `rn2(4)` gate while C scans at least one additional `fobj` entry. This points at missing/position-drifted special-level floor object state near the pet, not the basic dog-goal formula. |
...
**As of 2026-05-12: 55 / 11,406 public screens matched (~0.5%), 0/44 sessions fully passing**

---

## 🎯 Recommended Next Targets (Highest ROI first)

1. **Classify pet apparent-nearby/position parity after run turns (`monmove.c`/`dogmove.c`)** — `seed0116` now matches ordinary non-pet movement ownership through the FR 5616 `mtrack` roll and blocks at FR 5710, where C consumes the kitten `M2_WANDER` gate but JS starts `dog_goal()` object resistance. The likely owner is pet position/apparent target or remaining dog movement state, not command dispatch.
