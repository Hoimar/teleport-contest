...
1. **Classify `seed0383` FR 10491 swallowed-pet floor-object scan gap** — `seed0383` now matches the gnome levitation-boots pickup/wear delay, repeat ice-vortex `gulpmu()` damage, swallowed non-`u.ustuck` attack suppression, and later movement pass up to the next pet `dog_goal()`. The next mismatch is C scanning more floor objects via `obj_resists()` while JS starts `mcalcmove()`, so compare the pet search rectangle, retained `fobj` entries, and inventory/floor scan ordering before changing `dog_move()` candidate selection.
