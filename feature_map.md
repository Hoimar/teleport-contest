...
| Monster/object fill (mkfill, mineralize) | `mklev.c`, `mkobj.c`, `makemon.c`, `engrave.c`, `rumors.c` | `js/mklev.js` + `js/random_text.js` + `js/monster_data.js` + startup fast-forward scaffolds | 🟡 | `mkobj()` consumes upstream-derived class/object probability and material metadata. `rndmonst_adj()` now walks a generated `include/monsters.h` table with C's weighted reservoir RNG shape and numeric dungeon alignment (`A_NONE` for Dungeons of Doom); trap constants match `trap.h`; `occupied()` rejects trap squares; `CORPSE`/`STATUE`, trap-victim role corpse range, no-corpse retry, egg init, grid bug gender, multigen projectile quantity, kobold/giant/elven/centaur/troll/gnome `m_initweap()` slices, `S_QUANTMECH` inventory, `M2_GREEDY` money gates, shared offensive-item gate, `peace_minded()` predicates, and defensive-item creation now consume closer C RNG. Random graffiti uses NetHack-style padded data-file selection and `wipeout_text()`. `level_finalize_topology()` calls real `mineralize()`. Evidence: `seed0383` moved through vault fill, room fill, bigroom monster selection, special bigroom group init, selected equipment, peacefulness, and inventory gates to FR 5730; `seed8000` screen parity remains `23/23`. |
...
| Feature | C Reference | JS Implementation | Status | Notes |
|---|---|---|---|---|
| Monster placement (mklev fill) | `makemon.c`, `include/monsters.h`, `include/monflag.h` | `js/mklev.js:makemon()` + `js/monster_data.js` + hardcoded arrays in `allmain.js` | 🟡 | Random monster selection follows `rndmonst_adj()` reservoir sampling over generated `monsters.h` data and consumes `next_ident`, HP, `peace_minded()` predicate/call order, more selected-monster weapon/inventory gates, and retained level monster records. Exact selected-monster initialization/equipment, `collect_coords()` candidate accounting, and real movement AI are still incomplete; current `seed0383` blocker is special bigroom placement candidate count at FR 5730, and current `seed8000` blocker is live monster movement at FR 2985. |
...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `3716/16915` to `5788/16915`; vault fill, final special-room refill with gold merging, kobold/giant/elven/centaur/troll/gnome weapon init slices, multigen projectile quantity, shared offensive-item gate, defensive-item creation, `peace_minded()` predicate coverage, `S_QUANTMECH` inventory gate, debug `#levelchange`, debug level-teleport menu, partial `bigrm-12` special loading, special bigroom group init, general generated `monsters.h` reservoir sampling, and generated object retention are now modeled. Current blocker is `collect_coords()`/`enexto()` spatial candidate-count drift at FR 5730 (`rn2(17)` expected vs `rn2(24)` actual). |
...
1. **Classify special bigroom `collect_coords()` candidate drift** — `seed0383` now reaches FR 5730, where expected `rn2(17)` vs actual `rn2(24)` indicates JS has too many spatial candidates during placement after selected-monster init.
