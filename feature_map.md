...
| Monster/object fill (mkfill, mineralize) | `mklev.c`, `mkobj.c`, `makemon.c`, `engrave.c`, `rumors.c` | `js/mklev.js` + `js/random_text.js` + `js/monster_data.js` + startup fast-forward scaffolds | 🟡 | `mkobj()` consumes upstream-derived class/object probability and material metadata. `rndmonst_adj()` now walks a generated `include/monsters.h` table with C's weighted reservoir RNG shape and numeric dungeon alignment (`A_NONE` for Dungeons of Doom); trap constants match `trap.h`; `occupied()` rejects trap squares; `CORPSE`/`STATUE`, trap-victim role corpse range, no-corpse retry, egg init, grid bug gender, multigen projectile quantity, kobold/giant/elven/centaur/troll/gnome/ogre/default `m_initweap()` slices, mummy inventory, `S_QUANTMECH` inventory, `M2_GREEDY` money gates with minvent-shaped dice, adjusted-`m_lev` inventory gates, `mflags1` defensive-item predicates, golem fixed HP, shared offensive-item gate, amulet cursed-gate init, and `peace_minded()` predicates now consume closer C RNG. Random graffiti uses NetHack-style padded data-file selection and `wipeout_text()`. `level_finalize_topology()` calls real `mineralize()`, retains unburied mineralized gold/gems on `level.objects`, and skips buried gold/gems after kelp on most special levels. Evidence: `seed0116` now carries the expected nearby mineral objects into the first run turn, and `seed0383` still blocks in pet `dog_goal()` floor-object state at FR 9716. `seed8000` screen parity remains `23/23`. |
...
| Command dispatch (`rhack`) | `cmd.c:rhack()` | `js/cmd.js:rhack()` | 🟡 | Move commands, uppercase vi run commands, rest/wait (`.`), search (`s`), empty-square pickup messaging, `#pay` no-shopkeeper message, basic drop, put-on ring, zap-wand prompt/direction flow, wizard wishes for current object evidence, basic UI stubs, debug `^V` numeric/menu level teleport, and `#levelchange` prompt/level-gain RNG are present. Run commands now consume repeated movement/turn loops before the next input boundary, but stop/turn/lookaround rules are still approximate. Most commands remain unimplemented. |
...
| Per-step monster movement / regen RNG | `monmove.c`, `regen.c` | `js/monmove.js` + partial turn consumers | 🟡 | Per-step replay is no longer called from `moveloop_core()`, and the dead `fastforward_step()` exports have been removed from the seed replay modules. `makemon()` now preserves C `fmon` head-insertion order, `mcalcmove()` handles slow/fast speed states, fog clouds consume their `m_everyturn_effect()` vapor-cloud TTL from the `movemon_singlemon()` phase before the movement-budget check, uppercase run commands reuse the normal turn advancement loop, and ordinary non-pet monsters now take a minimal `dochug()` movement-opportunity path with post-`m_move()` `distfleeck()` plus `mtrack` backtracking rolls. Movement AI remains skeletal; `seed0116` now blocks at FR 5911 after the digging-wand action, where C has one more `exercise()` before pet movement while JS starts `distfleeck()`. `seed8000` remains complete on screens and blocks later in live monster movement, and `seed0383` remains in pet object/position state at FR 9716. |
...
| Feature | C Reference | JS Implementation | Status | Notes |
|---|---|---|---|---|
| Monster placement (mklev fill) | `makemon.c`, `include/monsters.h`, `include/monflag.h` | `js/mklev.js:makemon()` + `js/monster_data.js` + hardcoded arrays in `allmain.js` | 🟡 | Random monster selection follows `rndmonst_adj()` reservoir sampling over generated `monsters.h` data and consumes `next_ident`, adjusted HP including golem fixed-HP no-RNG cases, `peace_minded()` predicate/call order, broader selected-monster weapon/inventory gates, `mflags1` defensive-item predicates, greedy gold dice/object creation, retained level monster records, and `goodpos()` occupied-square rejection for placement even without `GP_AVOID_MONPOS`. Exact selected-monster initialization/equipment, some special-level placement/selection details, and real movement AI are still incomplete; current `seed0383` blocker is pet-arrival/search-rectangle drift after special bigroom setup, and current `seed8000` blocker is live monster movement at FR 2985. |
| Monster movement | `monmove.c` | `js/monmove.js` (partial) | 🟡 | Ordinary monsters now have movement budget ownership, post-`m_move()` `distfleeck()` recalc, a narrow adjacent-candidate `m_move()` skeleton, and retained `mtrack` backtracking rolls. Doors, attacks, item pickup, tunneling, special movement, traps, and full `mfndpos()` flags remain incomplete. |
| Combat | `fight.c` | — | 🔴 | |
| Pet behavior | `dog.c`, `dogmove.c` | `js/dog.js` + `js/mklev.js:makemon()` + `js/monmove.js` (partial) | 🟡 | Starting pet creation is partially ported for configured non-replay roles: role/preferred-pet selection, `MM_EDOG|NO_MINVENT`, near-hero `collect_coords()` ring shuffling, pet tame/peaceful setup, pre-inventory `edog->apport` initialization, `edog->whistletime`, and kitten/little-dog/pony metadata. Tame monster turns now enter a partial `dog_move()` path with C-like `M2_WANDER` short-circuit behavior, C-like neighbor scan order, `whappr` suppression of worse-candidate backtracking, `chcnt` reset after strictly better candidates, and old/new square redraws. `dog_goal()` now scans retained floor objects and hero inventory with `dogfood()`/`obj_resists()` guards for real numeric `otyp` objects; its apport branch has the C lighting/front-door predicate, `m_cansee()` via `clear_path()`, partial diet-aware food classification, rock-class `UNDEF` handling after the resistance roll, and pool/lava/boulder reachability gate. Level-teleport pet arrival now preserves the migrating tame monster's movement/data across `mklev()` instead of recreating a fixed-movement pet. Current `seed0116` blocker is after the run-command movement phase, at FR 5710: C evaluates the kitten's nearby `M2_WANDER` gate while JS sees the pet two squares away and starts floor-object resistance checks. Current `seed0383` blocker is missing additional retained/positioned floor objects after the second apport check at FR 9716. Hunger, naming, saddles, full object fetching/eating, migration details, and interactions remain incomplete. |
...
| seed0002-healer-reflection-drummer | 11 | 595 | 2026-05-12 | First visible screen mismatch unchanged at screen 11. Current lagging RNG evidence is `1251/27158`; first RNG mismatch is FR 1215 after generalized movement/object-state fixes. |
| seed0116-wizard-wear-shop | 80 | 127 | 2026-05-12 | Door-state aliasing, tutorial prompt lifecycle, rest command, debug `^V` prompt/Enter handling, static themed `des.map()`, irregular region bounds, corridor/vault accounting, random shop decision, level-teleport arrival placement, pet migration, normal Space handling, basic wear/takeoff/pickup/drop prompts, retained mineral objects, pet wanderer short-circuiting, partial `dog_goal()` object/inventory scans, `whappr`, uppercase `L` run handling, ordinary non-pet `m_move()`/`mtrack` ownership, persistent inventory letters, wished ring/digging-wand retention, `P` ring put-on prompts, and `z` wand selection/direction flow moved this evidence path to screen 80 and FR 5911. Current blocker is turn/movement phase ownership after a digging-wand zap: C consumes another `exercise()` before pet movement while JS enters `distfleeck()`, with a visible pet/map mismatch. Current lagging RNG is `5958/12562`. |
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-12 | Improved from `5788/16915` to `9914/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init and retained wish inventory, pet-arrival movement, fog-cloud every-turn vapor TTL, newest-first floor-object order, pre-inventory pet apport, `m_cansee()` clear-path visibility, partial pet food/reachability gates, and ordinary monster movement ownership are now modeled. Current blocker is FR 9716: after matching the kitten wanderer gate, six floor-object `obj_resists()` calls, and two failed apport rolls, JS reaches the follow-player `rn2(4)` gate while C scans at least one additional `fobj` entry. This points at missing/position-drifted special-level floor object state near the pet, not the basic dog-goal formula. |
| All other sessions | 0 | 10,442 | 2026-05-12 | Non-replay sessions now run general `init_objects()`, partial `role_init`, data-driven `init_dungeons()`, partial `u_init_misc`, starting pet placement, Wizard `ini_inv`, and broader mklev/monster equipment RNG. Many RNG prefixes improved, but first-screen parity still awaits broader role/race/inventory side effects, shop/special-room handling, save/restore, and hallucination display context. |

**As of 2026-05-12: 114 / 11,406 public screens matched (~1.0%), 0/44 sessions fully passing**

---

## 🎯 Recommended Next Targets (Highest ROI first)

1. **Classify post-zap exercise/monster phase order (`allmain.c`/`zap.c`/`dogmove.c`)** — `seed0116` now reaches the digging-wand direction action and blocks at FR 5911. C consumes a second `exercise()` after `zap_dig()` before pet movement; JS starts `distfleeck()` immediately. Verify whether the owner is remaining `weffects()` disclosure/wand learning, periodic `exerper()` timing, or the JS `advanceTurn()` phase order before moving RNG consumers.
2. **Complete `dog_goal()` object visibility/state (`dogmove.c`)** — `seed0383` still reaches FR 9716 after fog-cloud, movement-allocation, object-order, pre-inventory apport, clear-path visibility, and partial pet food/reachability parity; the next mismatch is missing floor objects/positions in the remaining `fobj` scan. Use `scripts/trace-dog-goal.mjs --scan --rng` before changing candidate movement.
3. **Classify remaining special-level monster/list effects** — `seed0383` crossed the prior fog-cloud phase-order blocker, but special-level monster ordering can still affect later movement and display once `dog_goal()` advances.
