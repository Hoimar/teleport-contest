...
| Per-step monster movement / regen RNG | `monmove.c`, `regen.c` | `js/monmove.js` + partial turn consumers | 🟡 | Per-step replay is no longer called from `moveloop_core()`, and the dead `fastforward_step()` exports have been removed from the seed replay modules. `makemon()` now preserves C `fmon` head-insertion order, `mcalcmove()` handles slow/fast speed states, fog clouds consume their `m_everyturn_effect()` vapor-cloud TTL from the `movemon_singlemon()` phase before the movement-budget check, and uppercase run commands reuse the normal turn advancement loop. Movement AI remains skeletal; `seed0116` now reaches the `L` run turn and blocks at FR 5573 where C has another `distfleeck()` before JS continues the next `dog_goal()` object scan. `seed8000` still blocks at FR 2985, and `seed0383` remains in pet object/position state at FR 9716. |
...
|---|---|---|---|---|
| seed8000-tourist-starter | **23** | 23 | 2026-05-12 | Screen parity preserved. First RNG mismatch remains FR 2985 in live monster movement (`distfleeck()` vs movement allocation); current lagging RNG is `3010/3130`. |
| seed0002-healer-reflection-drummer | 11 | 595 | 2026-05-12 | First visible screen mismatch unchanged at screen 11. Current lagging RNG evidence is `1247/27158`; first RNG mismatch is FR 1215 after generalized pet/object-state fixes. |
| seed0116-wizard-wear-shop | 17 | 127 | 2026-05-12 | Door-state aliasing, tutorial prompt lifecycle, rest command, debug `^V` prompt/Enter handling, static themed `des.map()`, irregular region bounds, corridor/vault accounting, random shop decision, level-teleport arrival placement, pet migration, normal Space handling, basic wear/takeoff prompts, retained mineral objects, pet wanderer short-circuiting, partial `dog_goal()` object/inventory scans, `whappr`, and uppercase `L` run handling moved this evidence path to screen 17 and FR 5573. Current blocker is live monster movement ownership during the internal run turns: C has another `distfleeck()` before JS continues dog-goal object scans; visible mismatch is the kitten one row south after `L`. Current lagging RNG is `5682/12562`. |
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-12 | Improved from `5788/16915` to `9877/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init and retained wish inventory, pet-arrival movement, fog-cloud every-turn vapor TTL, newest-first floor-object order, pre-inventory pet apport, `m_cansee()` clear-path visibility, and partial pet food/reachability gates are now modeled. Current blocker is FR 9716: after matching the kitten wanderer gate, six floor-object `obj_resists()` calls, and two failed apport rolls, JS reaches the follow-player `rn2(4)` gate while C scans at least one additional `fobj` entry. This points at missing/position-drifted special-level floor object state near the pet, not the basic dog-goal formula. |
...
**As of 2026-05-12: 51 / 11,406 public screens matched (~0.4%), 0/44 sessions fully passing**

---

## 🎯 Recommended Next Targets (Highest ROI first)

1. **Classify live monster movement ownership during run turns (`monmove.c`/`dogmove.c`)** — `seed0116` now reaches uppercase `L` run handling and matches the first internal run turn through `dog_goal()`, but diverges at FR 5573 where C has an additional `distfleeck()` before JS continues pet object scanning. The likely owner is broader `movemon_singlemon()` readiness/state for non-pet monsters, not another command-dispatch patch.
