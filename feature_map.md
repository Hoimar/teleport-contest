...
| Combat | `fight.c` | — | 🔴 | |
| Pet behavior | `dog.c`, `dogmove.c` | `js/dog.js` + `js/mklev.js:makemon()` + `js/monmove.js` (partial) | 🟡 | Starting pet creation is partially ported for configured non-replay roles: role/preferred-pet selection, `MM_EDOG|NO_MINVENT`, near-hero `collect_coords()` ring shuffling, pet tame/peaceful setup, and kitten/little-dog/pony metadata. Tame monster turns now enter a partial `dog_move()` path with the upstream kitten/pony `M2_WANDER` gate and C-like neighbor scan order. `dog_goal()` now scans retained floor objects and hero inventory with `dogfood()`/`obj_resists()` guards for real numeric `otyp` objects, and its apport branch has the C lighting/visibility front door instead of an unconditional object-visible predicate. Level-teleport pet arrival now preserves the migrating tame monster's movement/data across `mklev()` instead of recreating a fixed-movement pet. Current `seed0116` blocker is missing three object-resistance calls after retained Wizard inventory, likely nearby floor/shop object state; current `seed0383` blocker is the next `dog_goal()` apport/object scan ordering after five floor-object resistance checks. Hunger, naming, saddles, full object fetching/eating, migration details, and interactions remain incomplete. |
...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `5788/16915` to `9882/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init, pet-arrival movement, fog-cloud every-turn vapor TTL, and newest-first floor-object order are now modeled. Current blocker moved from FR 9666 to FR 9713: after matching fog-cloud TTL, all 36 movement-allocation `rn2(12)` rolls, random-monster/gethungry/slippery-finger gates, and five `dog_goal()` object-resistance checks, JS scans one more object where C rolls the apport `rn2(8)`. |
...
1. **Complete `dog_goal()` object visibility/state (`dogmove.c`)** — `seed0383` now reaches FR 9713 after fog-cloud and movement-allocation parity; the next mismatch is the apport visibility/object-scan boundary after five floor-object `obj_resists()` calls. `seed0116` still needs three missing `obj_resists()` calls before the pet chooses its square, likely incomplete floor/object state.
