...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `5788/16915` to `10004/16915` this loop; `bigrm-12` map alignment, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, and golem fixed HP are now modeled. Current blocker is FR 9383 (`rn2(2)` expected vs `rn2(10)` actual) after deeper special bigroom monster/object initialization. |
...
1. **Continue special bigroom monster/object initialization** — `seed0383` now reaches FR 9383 after map alignment, more selected-monster equipment/inventory, gold, and golem HP parity; classify the new `rn2(2)` vs `rn2(10)` blocker before editing.
