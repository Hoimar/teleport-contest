...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `5788/16915` to `9895/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init, and pet-arrival movement are now modeled. Current blocker is FR 9666 (`rn2(3)` expected vs `rn2(12)` actual), likely missing `create_gas_cloud()`/region-side effect before monster movement accrual. |
...
1. **Classify `create_gas_cloud()` region side effect** — `seed0383` now reaches FR 9666 after bigroom special loading, arrival, and wizard wish object initialization; the next missing consumer is `rn2(3)` at `region.c:create_gas_cloud()` before monster movement accrual.
