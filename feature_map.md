...
| `o_init` (object type shuffle) | `o_init.c` | `js/o_init.js` | 🟡 | General RNG shape for gem colors, description shuffle ranges, and `WAN_NOTHING` direction is implemented for non-replay sessions. Runtime `oc_color` and `oc_material` swaps are now retained for map display and material predicates. Actual shuffled description/name identity, toughness, discovery, and user-facing description lookup are still incomplete. |
...
| Per-step monster movement / regen RNG | `monmove.c`, `regen.c`, `were.c` | `js/monmove.js` + partial turn consumers | 🟡 | Per-step replay is no longer called from `moveloop_core()`, and the dead `fastforward_step()` exports have been removed from the seed replay modules. `makemon()` now preserves C `fmon` head-insertion order, `movemon()` snapshots the pass like `iter_mons_safe()`, `mcalcmove()` handles slow/fast speed states, `mcalcdistress()` owns the human-form werecreature `were_change()` denominator gate before movement allocation, damaged-hero `regen_hp()` owns the post-generation `rn2(100)` turn-tail roll, fog clouds retain vapor regions, keep zero-TTL regions visible until the next region tick, and age after the final monster movement pass for the turn, uppercase run commands reuse the normal turn advancement loop, sleeping/frozen monsters and object/furniture-appearing hiders spend movement without entering `distfleeck()`, and ordinary non-pet monsters now take a minimal `dochug()`/`m_move()` path with post-`m_move()` `distfleeck()`, swallowed-bystander `MMOVE_MOVED`, `set_apparxy()`-style hero targeting, swallowed hero coordinates pinned to `u.ustuck`, peaceful/light approach gates, `mtrack` backtracking rolls, partial `mfndpos()` terrain eligibility for flying/swimming/pool/lava squares, minimal `m_search_items()`/`mpickstuff()` collector pickup with boot `m_dowear()` delay, and a constrained post-move `mattacku()` front door for physical and current engulf evidence. Movement AI remains skeletal, but `seed0116` now has full RNG parity (`12562/12562`) and only a display attr mismatch remains. `seed8000` remains complete on screens, and `seed0383` now blocks at FR 10608 after the swallowed-pet `dog_goal()` and fog-region lifetime gates; the next owner is command/prompt alignment before swallowed hero `uhitm()`, not another dog-goal floor-object scan. |
...
| Feature | C Reference | JS Implementation | Status | Notes |
|---|---|---|---|---|
| Object type table / `o_init` shuffle | `objects.c`, `o_init.c` | `js/object_data.js` + `js/o_init.js` | 🟡 | Static upstream object class/probability/charge/direction/material/color metadata is available for `mkobj` selection, erosion eligibility, and map display. `init_objects()` now consumes the general C description-shuffle RNG shape for non-replay sessions and retains shuffled color/material state, reducing seed-specific startup replay reliance and fixing the current seed0116 ring color. Runtime object-description/name lookup, toughness swaps, artifact selection/origin tracking, and item identity parity are still missing; current wished-object evidence carries explicit appearance names for the shuffled teleport-control ring and digging wand until the real description table is wired. |
...
1. **Classify `seed0383` FR 10608 swallowed-command / wear-prompt alignment** — `seed0383` now matches the swallowed pet `dog_goal()` floor/inventory scan and the fog-cloud zero-TTL lifetime gate. The next mismatch is C handling key `h` as swallowed hero `uhitm()` after zero-RNG prompt/message state, while JS is still treating the key stream as ordinary turn movement. Align `W`/`n` wear selection, `--More--` screens, and `#wizintrinsic` prompt state before enabling a swallowed hero attack front door.
2. **Classify Soko screen 109 remaining object identity/color drift** — seed0116 now has full RNG parity and runtime shuffled `o_init` colors. The remaining three visible attr cells are live object type/color mismatches for two potions and an arrow stack; trace them to Soko/trap-victim/special-level object creation rather than terrain/display color.
