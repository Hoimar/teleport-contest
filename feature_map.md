...
| Per-step monster movement / regen RNG | `monmove.c`, `regen.c`, `were.c` | `js/monmove.js` + partial turn consumers | 🟡 | Per-step replay is no longer called from `moveloop_core()`, and the dead `fastforward_step()` exports have been removed from the seed replay modules. `makemon()` now preserves C `fmon` head-insertion order, `mcalcmove()` handles slow/fast speed states, `mcalcdistress()` owns the human-form werecreature `were_change()` denominator gate before movement allocation, fog clouds consume their `m_everyturn_effect()` vapor-cloud TTL from the `movemon_singlemon()` phase before the movement-budget check, uppercase run commands reuse the normal turn advancement loop, sleeping/frozen monsters and object/furniture-appearing hiders spend movement without entering `distfleeck()`, and ordinary non-pet monsters now take a minimal `dochug()` movement-opportunity path with post-`m_move()` `distfleeck()` plus `mtrack` backtracking rolls. Movement AI remains skeletal, but `seed0116` now has full RNG parity (`12562/12562`) and only a display attr mismatch remains. `seed8000` remains complete on screens, and `seed0383` remains in pet object/position state at FR 9716. |
...
| Monster movement | `monmove.c`, `were.c` | `js/monmove.js` (partial) | 🟡 | Ordinary monsters now have movement budget ownership, post-`m_move()` `distfleeck()` recalc, a narrow adjacent-candidate `m_move()` skeleton, retained `mtrack` backtracking rolls, the pre-allocation `mcalcdistress()`/human-were `were_change()` gate, and C-shaped no-RNG front doors for frozen/sleeping monsters plus hidden mimics. Doors, attacks, item pickup, tunneling, special movement, traps, transformations, and full `mfndpos()` flags remain incomplete. Current seed0116 evidence now has complete RNG parity, so remaining seed0116 work is display/object attr state rather than live movement. |
| Combat | `fight.c` | — | 🔴 | |
...
1. **Complete `dog_goal()` object visibility/state (`dogmove.c`)** — `seed0383` still reaches FR 9716 after fog-cloud, movement-allocation, object-order, pre-inventory apport, clear-path visibility, and partial pet food/reachability parity; the next mismatch is missing floor objects/positions in the remaining `fobj` scan. Use `scripts/trace-dog-goal.mjs --scan --rng` before changing candidate movement.
