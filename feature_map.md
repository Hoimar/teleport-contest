...
| Special level loading (Lua specials) | `sp_lev.c` | `js/mklev.js` static themed-map/bigroom slices | 🔴 | Full special level support is still absent. Static themed `des.map()` fragments now use exact upstream map literals for implemented shapes, enable `in_mk_themerooms` failure semantics, create irregular `filler_region()` rooms from flood-filled floor bounds, and use C-like irregular door search. `bigrm-12` now dispatches before ordinary level generation, consumes the random variant, Lua shuffle/init, applies the C-centered map offset (`x=3,y=1` for the 75x19 slice), runs scripted percent gates, stair/object/trap/monster placement, consumes the post-load `flip_level_rnd()` horizontal gate for `noflipy`, and skips special-level buried minerals after kelp. Debug level teleport uses `place_lregion()` with monster-occupied teleport candidate rejection. Current `seed0383` blocker is after arrival/wish handling at FR 9666 (`create_gas_cloud()` TTL), not the earlier bigroom placement/init slices. |
...
| Object placement on map | `mkobj.c:mksobj()` | `js/mklev.js:mksobj_at()` / `mkobj_at()` + legacy hardcoded arrays in `allmain.js` | 🟡 | Generated `_at` object creation now retains non-gold objects on `level.objects` with class glyph state, uses C's newest-first `fobj` order, and `mkcorpstat()` now places created corpses/statues at the supplied coordinates instead of only consuming their init RNG. Seed0002 still has legacy hardcoded startup objects, and object stacking/descriptions/material swaps/container contents remain incomplete. Evidence: `seed0116` dog blocker moved to `dog_goal()` object reachability/resistance rather than missing the pet wanderer gate; retained corpse/statue state and C-like floor-list order reduce hidden object-state debt for later dog/display/pickup work. |
...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-11 | Improved from `5788/16915` to `9889/16915` this loop; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init, pet-arrival movement, fog-cloud every-turn vapor TTL, newest-first floor-object order, pre-inventory pet apport, `m_cansee()` clear-path visibility, and partial pet food/reachability gates are now modeled. Current blocker moved from FR 9666 to FR 9716: after matching the two `dog_goal()` apport checks, C still scans additional floor objects while JS has exhausted nearby retained objects and starts dog movement selection. |
