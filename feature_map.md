...
| Feature | C Reference | JS Implementation | Status | Notes |
|---|---|---|---|---|
| Monster placement (mklev fill) | `makemon.c`, `include/monsters.h`, `include/monflag.h` | `js/mklev.js:makemon()` + `js/monster_data.js` + hardcoded arrays in `allmain.js` | 🟡 | Random monster selection follows `rndmonst_adj()` reservoir sampling over generated `monsters.h` data and consumes `next_ident`, adjusted instance level/HP including golem fixed-HP no-RNG cases, `peace_minded()` predicate/call order, broader selected-monster weapon/inventory gates, `mflags1` defensive-item predicates, leprechaun gold/minvent state, greedy gold dice/object creation, `rnd_misc_item()` slices, retained level monster records, `MM_ASLEEP`, Soko mimic boulder appearance state, spider/snake hide-under object creation during mklev, and `goodpos()` occupied-square rejection for placement even without `GP_AVOID_MONPOS`. Exact selected-monster initialization/equipment, some special-level placement/selection details, and real movement AI are still incomplete; current `seed0116` blocker is display attr drift at screen 109, current `seed0383` blocker is live movement after post-gnome pet passes, and current `seed8000` blocker is live monster movement at FR 2985. |
...
| seed0383-wizard-hallucinate | 0 | 219 | 2026-05-12 | Current lagging RNG is `10527/16915`; `bigrm-12` map alignment, post-load flip, special mineralize skip, default/ogre weapon init, mummy inventory, adjusted-`m_lev` item gates, retained monster `m_lev`/`mhpmax`, `mflags1` defensive-item front door, greedy gold amount/object creation, golem fixed HP, teleport-region candidate rejection, wizard wish object init and retained wish inventory, zero-time `#wizintrinsic` hallucination prompt state, Wizard startup/worn-state handling, initial inventory stack merging, `P`/`W` wearable selection timing with delayed armor occupation, pet-arrival movement, fog-cloud every-turn vapor region retention, newest-first floor-object order, pre-inventory pet apport, `m_cansee()` clear-path visibility, partial pet food/reachability gates, pet ranged target scoring, ordinary monster movement front doors, flying/swimming `mfndpos()` terrain candidates, pet candidate melee with adjusted-level balking, and hero occupied-monster movement front door are now modeled. Current blocker is FR 10082 after the post-gnome pet pass: C enters the next ordinary monster `distfleeck()` while JS consumes an extra `rn2(3)` movement gate, so the next owner is ordinary monster movement predicate/front-door state after the pet move. |
...
1. **Classify `seed0383` FR 10082 ordinary-monster front door** — `seed0383` now reaches FR 10082 after command, delayed wear/occupation, inventory-stack, dog-goal, pet ranged scoring, fog-region, peaceful/light front doors, flying `mfndpos()` terrain accounting, the first pet melee/gnome kill, and adjusted-level pet attack balking. The next mismatch is C `distfleeck()` for the next ordinary monster versus JS an extra `rn2(3)` movement gate. Focus on the specific monster's movement predicate, approach gate, and spatial state rather than adding another pet combat rule.
