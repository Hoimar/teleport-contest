...
- While the leaderboard divergence is being rooted out, use `pre-leaderboard-debug` as the temporary long-running work branch unless the user names another branch.
- Do not create extra work branches, mutate `main`, push, or force-push during the loop unless the user explicitly asks for that operation. `main` is the leaderboard promotion lane.
- Make local commits on the work branch after coherent, verified implementation improvements. Failed experiments do not need commits; either revert the experiment, narrow it into a real fix, or leave it only as a scratch note.
- If the branch state differs from the policy above, record the current branch and proceed only if it is clearly the user-requested branch.
