...
| Dehacker / Simplifier | Removing or merging harness and replay debt | Replay, override, seed, stale-doc, duplicated-tool, and debug debt |
...
porter hypothesis." For marathon loops, periodically switch to the
Dehacker/Simplifier role and ask whether recent forward-only changes can be
deleted, merged into an existing script, or replaced by live subsystem code.
The agent should still use the same scripts and runbooks.
